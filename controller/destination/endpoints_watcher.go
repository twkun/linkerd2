...
type servicePort struct {
	...
	addresses  map[common.TcpAddress]*v1.Pod
	podLister  corelisters.PodLister
...
func (sp *servicePort) deleteEndpoints() {
	...
	sp.addresses = map[common.TcpAddress]*v1.Pod{}
}
...
func (sp *servicePort) updateAddresses(endpoints *v1.Endpoints, port intstr.IntOrString) {
	...
	newAddresses := make([]common.TcpAddress, 0)
	...
	if len(newAddresses) == 0 {
		...
		oldAddresses := make([]common.TcpAddress, 0)
		for a := range sp.addresses {
			oldAddresses = append(oldAddresses, a)
		}
		add, remove := addr.DiffAddresses(oldAddresses, newAddresses)
		addAddresses := make(map[common.TcpAddress]*v1.Pod)
		for _, a := range add {
...
func (sp *servicePort) addressMap(endpoints *v1.Endpoints, port intstr.IntOrString) map[common.TcpAddress]*v1.Pod {
	addresses := make(map[common.TcpAddress]*v1.Pod)
	...
	for _, subset := range endpoints.Subsets {
		for _, address := range subset.Addresses {
			...
			addresses[common.TcpAddress{Ip: ip, Port: portNum}] = pod
		}
	}
